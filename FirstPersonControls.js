...
import { MathUtils, Spherical, Vector3, Vector2 } from 'three';
...
class FirstPersonControls {
  constructor(object, domElement) {
    ...
    // base acceleration
    this.accel = 1000; // originally set at 1
    // increases while a button is held down
    this.accelMultiplier = 5; // originally set at 0
    this.decel = this.mobile ? 0.999 : 0.9;
    this.minx = 1; // originally set at 0.1
    this.dx = 0;
    this.dy = 0;
    this.dz = 0;
    ...
    this.heightCoef = 3.0; // originally set at 1.0
    this.heightMin = 0.0;
    this.heightMax = 1.0;
    ...
    this.autoSpeedFactor = 10.0; // originally set at 0.0
    ...
    this.drag = function (x, y) {
      if (
        Math.abs(this.mouseXinitial - x) < NON_DRAG_DISTANCE &&
        Math.abs(this.mouseYinitial - y) < NON_DRAG_DISTANCE
      ) {
        return;
      }
      this.dragging = true;
