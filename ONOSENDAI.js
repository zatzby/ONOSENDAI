...
import {
  colors,
  whiteMaterial,
  expandedCubeMaterial,
  expandedBookmarkedCubeMaterial,
  connectToRoot,
  visitedMaterial,
  sunMaterial,
  connectToReplies,
...
function init() {
  ...
  function setupOrtho(tex) {
    ...
    lilgrid.setRotationFromQuaternion(
      camera.getWorldQuaternion(new THREE.Quaternion()).invert()
    );
  }
  ...
  // setup speed lines
  speedLines = [];
  (speedLinesNearDist = 8), (speedLinesFarDist = -10);
  speedLineCount = 2;
  ...
  const environmentLightLeft = new THREE.DirectionalLight(
    colors.ENVIRONMENT_LEFT_COLOR,
    0.8
  );
  environmentLightLeft.position.set(1, 1, 1);
  scene.add(environmentLightLeft);
  const environmentLightRight = new THREE.DirectionalLight(
    colors.ENVIRONMENT_RIGHT_COLOR,
    0.8
  );
  ...
  const gridtop = new THREE.GridHelper(
    WORLD_SCALE / 2,
    100,
    colors.LOGO_BLUE,
    colors.LOGO_BLUE
  ...
  const operatorLampLeft = new THREE.PointLight(
    colors.LEFT_LAMP_COLOR,
    OPERATOR_LAMP_STRENGTH,
    100,
    1
  );
  const operatorLampCenter = new THREE.PointLight(
    colors.CENTER_LAMP_COLOR,
    OPERATOR_LAMP_STRENGTH,
    500,
    1
  );
  const operatorLampRight = new THREE.PointLight(
    colors.RIGHT_LAMP_COLOR,
    OPERATOR_LAMP_STRENGTH,
    100,
    1
  );
...
function render() {
  ...
  ccs.innerHTML = `<span>[</span>${Math.floor(
    camera.position.x
  )}x<span>][</span>${Math.floor(
    camera.position.y
  )}y<span>][</span>${Math.floor(camera.position.z)}z<span>]</span>`;
...
function updateCycle(controls) {
  ...
  let newPos = new THREE.Vector3(
    node.position.x,
    node.position.y + camdist / 2,
    node.position.z + camdist
  );
...
function updateSelectedNote(controls) {
  if (
    controls.mouseUpThisFrame &&
    intersected &&
    selected?.intersected !== intersected
  ) {
    ...
    if (selected) {
      ...
      mesh.rotation.y = 0;
      mesh.rotation.x = 0;
      mesh.material = bookmarkedEvents.hasOwnProperty(mesh.userData.event.id)
        ? bookmarkedMaterial
        : visitedMaterial;
...
function updateUniverse(controls) {
  ...
  if (scalar) {
    ...
    universe.children.forEach(c =>
      c.scale.set(1 / newScale, 1 / newScale, 1 / newScale)
    );
...
function animateReticle(delta) {
  ...
  if (intersected) {
    ...
    if (Math.abs(reticle.material.rotation) < 0.0001)
...
function animateSpeedLines(controls) {
  ...
  speedLines.slice(0, 2).forEach(l => {
    l.material.opacity =
      Math.abs(speed) < 200
        ? Math.min(Math.max(0.25 + Math.abs(speed / 10), 0.25), 1)
        : Math.min(1, Math.max(200 / Math.abs(speed), 0.15));
    l.position.z += -dz / 100;
...
function showThread(event) {
  if (event) {
    ...
    let rootEvent =
      event.tags[0] && event.tags[0][0] === 'e' && event.tags[0][1]
        ? event.tags[0][1]
        : false;
    if (rootEvent && isLoaded(rootEvent)) {
      // connect activated event cube to root event
      connectNotes(
        selected.intersected.object.position,
        loadedEvents[rootEvent].noteMesh.position
      );
    }
    if (
      !rootEvent &&
      (event.tags?.length == 0 || (event.tags[0] && event.tags[0][0] !== 'e'))
    ) {
      // console.log('checking for replies to',event.id)
      // see if there are replies
      let replies = Object.keys(loadedEvents)
        .map(e => loadedEvents[e])
        .filter(e => e.tags[0] && e.tags[0][1] && e.tags[0][1] === event.id);
      replies.forEach(r =>
        connectNotes(
          selected.intersected.object.position,
          r.noteMesh.position,
...
export const visualizeNote = (event, coords) => {
  ...
  noteMesh.position.set(coords[0], coords[1], coords[2]);
